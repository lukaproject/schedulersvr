...
type (
	...
	Task struct {
		...
		TaskType   string         `db:"task_type"`   // task type
		...
		BeginTime  sql.NullInt64  `db:"begin_time"`  // the time (ms) of the user begin the task
		EndTime    sql.NullInt64  `db:"end_time"`    // the time (ms) of the user finished the task
		Status     sql.NullString `db:"status"`      // the task status
		WorkerId   sql.NullInt64  `db:"worker_id"`   // the worker_id which execute this task
	}
)
